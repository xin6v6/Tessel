...
| `bun run acceptance` | 端到端验收（以你身份 DM bot，见 `.env` 的验收 agent 段） |
...
Tessel 持久化整段 graph run（含 `state.messages`），让 bot 跨次 invoke 看到历史。底层是 `SqliteGraphStore`（`src/graph/store.ts`，用 `bun:sqlite`）。消息是普通 TS 对象，`JSON.stringify` 直接落盘。
...
超过 30 条 message 时，supervisor 只把最近 20 条传给 LLM。`state.messages` 本身不裁剪——store 仍保留全部，便于后续按 speaker 加权裁剪（Step 1.1）。
